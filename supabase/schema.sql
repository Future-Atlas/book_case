...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE profile_id = NEW.profile_id AND book_id = NEW.book_id
    ) THEN
        RAISE EXCEPTION 'duplicate_book_post' USING ERRCODE = 'P0001';
    END IF;
    RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS prevent_duplicate_book_post_before_insert ON public.posts;
CREATE TRIGGER prevent_duplicate_book_post_before_insert
BEFORE INSERT ON public.posts
FOR EACH ROW EXECUTE FUNCTION public.prevent_duplicate_book_post();
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM public.posts
        WHERE profile_id = OLD.profile_id AND book_id = OLD.book_id
    ) THEN
...
DROP TRIGGER IF EXISTS remove_favorite_after_post_delete_trigger ON public.posts;
CREATE TRIGGER remove_favorite_after_post_delete_trigger
AFTER DELETE ON public.posts
FOR EACH ROW EXECUTE FUNCTION public.remove_favorite_after_post_delete();
...
-- Favorites are intentionally capped at twelve books per profile.
CREATE OR REPLACE FUNCTION public.enforce_favorites_limit()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
...
INSERT INTO public.adult_content_terms (term) VALUES
    ('成人向け'), ('成人指定'), ('成人雑誌'), ('成年向け'),
    ('成年コミック'), ('成人コミック'), ('18禁'), ('r18'), ('r-18'),
    ('アダルト'), ('エロ本'), ('ポルノ'), ('hentai'), ('pornography'),
    ('pornographic'), ('adult magazine'), ('adults only'),
    ('sexually explicit')
...
DROP TRIGGER IF EXISTS classify_post_age_restriction_trigger ON public.posts;
CREATE TRIGGER classify_post_age_restriction_trigger
    BEFORE INSERT OR UPDATE OF
        book_title, book_author, book_publisher, book_description, comment
    ON public.posts
    FOR EACH ROW
...
CREATE POLICY "Allow authenticated users to insert posts"
    ...
    WITH CHECK (
        ...
        AND (is_age_restricted = false OR public.current_user_is_adult())
    );
...
-- Keep keyed identity fingerprints after an administrator deletes an account,
-- so the same person cannot immediately register again with the same contact data.
...
-- Keep completed-book shelves synchronized with posts.
...
-- Guardian consent declaration for users under 18.
...
BEGIN
    ...
    INSERT INTO public.account_details (
        profile_id, full_name, birth_date, phone_number,
        guardian_consent_declared_at, guardian_consent_terms_version
    ) VALUES (
        current_profile_id, btrim(p_full_name), p_birth_date, normalized_phone,
...
-- Authentication > Users has no custom columns, so expose both public
-- identity values through its existing Display name column.
CREATE OR REPLACE FUNCTION public.sync_profile_to_auth_metadata()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
